...
def process_euclidian_absolute_speeds(data_set, config):
    ...
    gaze_points = np.concatenate(
        (
            data_set.x.reshape(1, nb_samples),
            data_set.y.reshape(1, nb_samples),
            data_set.z.reshape(1, nb_samples),
        ),
        axis=0,
    )
...
def process_speed_components(data_set, config):
    ...
    g_p = np.concatenate(
        (
            data_set.x.reshape(1, nb_s),
            data_set.y.reshape(1, nb_s),
            data_set.z.reshape(1, nb_s),
        ),
        axis=0,
    )
