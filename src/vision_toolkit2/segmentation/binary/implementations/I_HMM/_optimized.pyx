...
def process_IHMM(data_set, config, segmentation_config):
    """
    I-HMM algorithm.
    """
    cdef double[:] a_s = data_set.absolute_speed

    cdef double i_low_vel  = segmentation_config.ihmm.init_low_velocity
    cdef double i_high_vel = segmentation_config.ihmm.init_high_velocity
    cdef double i_var      = segmentation_config.ihmm.init_variance

    cdef int n_iter = segmentation_config.ihmm.nb_iters
    cdef int s_f    = data_set.min_config.sampling_frequency
    ...
    i_fix = np.array([False]*data_set.min_config.nb_samples)
    i_fix[wi_fix] = True
    ...
    i_fix = np.array([True]*data_set.min_config.nb_samples)
    for s_int in s_ints:
    ...
    i_lab = np.array([False]*data_set.min_config.nb_samples)
    for f_int in f_ints:
