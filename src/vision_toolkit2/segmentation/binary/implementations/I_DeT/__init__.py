...
def process_impl(s, config):
    ...
    g_npts = np.concatenate((
        s.x.reshape(1, n_s).astype(np.float64),
        s.y.reshape(1, n_s).astype(np.float64),
        s.z.reshape(1, n_s).astype(np.float64)
    ), axis=0)
    ...
    C_clus = []
    avlb = {i: True for i in range(n_s)}
    ...
    if config.verbose:
        print("   Saccadic intervals identified with minimum duration: {s_du} sec".format(
            s_du=config.min_sac_duration))

    i_fix = np.ones(n_s, dtype=bool)
    ...
    if config.verbose:
        print("   Close saccadic intervals merged with duration threshold: {f_du} sec".format(
    ...
    if config.verbose:
        print("   Fixations ans saccades identified using availability status threshold: {s_th}".format(
