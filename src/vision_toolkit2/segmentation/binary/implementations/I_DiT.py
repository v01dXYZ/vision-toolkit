...
from vision_toolkit2.config import I_DiT, Segmentation
...
def process_impl(
    ...
    t_du = int(np.ceil(config.segmentation.i_dit.window_duration * s_f))
    t_di = config.segmentation.i_dit.dispersion_threshold
...
def default_config_impl(config, vf_diag):
    if config.distance_type == "euclidean":
        ...
        return Config(
            segmentation=Segmentation(idit_config),
        )
