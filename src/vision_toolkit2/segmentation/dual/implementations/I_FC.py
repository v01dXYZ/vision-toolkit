...
from Vision.segmentation_src.utils.segmentation_utils import (
    interval_merging,
    centroids_from_ints,
)
from Vision.segmentation_src.utils.velocity_distance_factory import (
...
def pre_process_IFC(data_set, config):
    ...
    assert config["distance_type"] == "angular", (
        "'Distance type' must be set to 'angular"
    )
    ...
    w_c_100_m = np.array(
        [
            theta_coord[
                :,
                np.minimum(
                    np.maximum(0, np.arange(i - n_50, i + n_50 + 1, dtype=int)), n_s - 1
                ),
            ]
            for i in range(n_s)
        ]
    ...
    w_c_22_sd = np.array(
        [
            suc_dir[
                np.minimum(
                    np.maximum(0, np.arange(i - n_11, i + n_11 + 1, dtype=int)), n_s - 1
                )
            ]
            for i in range(n_s)
        ]
    ...
    disp_v = (
        np.max(w_c_100_m[:, 0, :], axis=1)
        - np.min(w_c_100_m[:, 0, :], axis=1)
        + np.max(w_c_100_m[:, 1, :], axis=1)
        - np.min(w_c_100_m[:, 1, :], axis=1)
    )
    ...
    r_mat = np.array([np.cos(w_c_22_sd), np.sin(w_c_22_sd)])
    ...
    for i in range(n_s):
        ...
        bcea_v[i] = bcea(theta_coord[:, w_c_100], bcea_p, std_v[i])
    ...
    if i2mc:
        ...
        while i + t_mo < n_s:
            ...
            X_1 = np.concatenate(
                (t_x[w_c].reshape(n_wc, 1), t_y[w_c].reshape(n_wc, 1)), axis=1
            )
            ...
            X_2 = np.concatenate(
                (t_x[w_c][::2].reshape(n_wc_2, 1), t_y[w_c][::2].reshape(n_wc_2, 1)),
                axis=1,
            )
            ...
            X_4 = np.concatenate(
                (t_x[w_c][::4].reshape(n_wc_4, 1), t_y[w_c][::4].reshape(n_wc_4, 1)),
                axis=1,
            )
...
def process_IFC(data_set, config):
    ...
    if classifier != "rf":
        ...
        norm_ = np.load(path)
        feature_mat, _, _ = standard_normalization(
            feature_mat, mu=norm_[0], sigma=norm_[1]
        )
