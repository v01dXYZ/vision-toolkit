...
def pre_process_IHOV(data_set, config):
    ...
    coord = np.concatenate((x_array.reshape(1, n_s), y_array.reshape(1, n_s)), axis=0)
    ...
    w_2_m = np.array(
        [
            coord[:, np.minimum(n_s - 1, np.arange(i + 1, i + t_du + 1, dtype=int))]
            - coord[:, i].reshape(2, 1)
            for i in range(n_s)
        ]
    )
    ...
    w_3_m = np.array(
        [
            coord[
                :,
                np.minimum(n_s - 1, np.flip(np.arange(i + 1, i + t_du + 1, dtype=int))),
            ]
            - coord[:, np.maximum(0, np.arange(i - t_du, i, dtype=int))]
            for i in range(n_s)
        ]
...
def comp_angles(v_mat):
    ...
    dir_[_p] = (180 / np.pi) * np.arccos(np.divide(v_mat[0, :][_p], n_p, where=n_p > 0))
    ...
    dir_[_m] = (180 / np.pi) * (
        2 * np.pi - np.arccos(np.divide(v_mat[0, :][_m], n_m, where=n_m > 0))
    )
...
def process_IHOV(data_set, config):
    ...
    clf = joblib.load(
        "vision_toolkit/segmentation/segmentation_algorithms/trained_models/{sm}/i_{cl}_{task}.joblib".format(
