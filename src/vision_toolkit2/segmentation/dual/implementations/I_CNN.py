...
class CNN2D(Module):
    def __init__(self, numChannels, classes):
        ...
        self.conv2 = Conv2d(in_channels=64, out_channels=128, kernel_size=(1, 5))
        self.relu2 = ReLU()
        self.maxpool2 = MaxPool2d(
            kernel_size=(1, 2),
        )  # stride=(2, 2))

        self.conv3 = Conv2d(in_channels=128, out_channels=256, kernel_size=(1, 5))
        self.relu3 = ReLU()
...
def pre_process_ICNN_spdir(data_set, config):
    ...
    data = np.concatenate(
        (suc_dir.reshape(1, n_s), a_sp.reshape(1, n_s), time.reshape(1, n_s)), axis=0
    )
...
def pre_process_ICNN(data_set, config):
    """
...
def train_cnn1d(dataset, labels, config):
    ...
    training_data = pre_process_ICNN(dataset, config)
    ...
    train_loader = DataLoader(
        dataset, batch_size=config["ICNN_batch_size"], shuffle=True
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = CNN1D(
        numChannels=input_channels, classes=num_classes, input_length=input_length
    ).to(device)
    criterion = nn.NLLLoss()
    optimizer = optim.Adam(model.parameters(), lr=config["ICNN_learning_rate"])
    ...
    path = "vision_toolkit/segmentation/segmentation_algorithms/trained_models/I_CNN/i_cnn_{task}.pt".format(
        task=config["task"]
    )
...
def process_ICNN(data_set, config):
    if config["verbose"]:
    ...
    model = CNN1D(
        numChannels=2, classes=classes, input_length=config["ICNN_temporal_window_size"]
    )
    model.load_state_dict(torch.load(path))
    model.eval()

    w_data = pre_process_ICNN(data_set, config)

    tensor_test_X = torch.tensor(w_data, dtype=torch.float32)
    testDataLoader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(tensor_test_X),
    ...
    testDataLoader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(tensor_test_X), batch_size=1024
    )
    ...
    if task == "binary":
        ...
        f_ints = interval_merging(wi_fix, min_int_size=config["min_int_size"])
        ...
        s_ints = interval_merging(wi_sac, min_int_size=config["min_int_size"])
